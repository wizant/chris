...
if( empty( $config[ 'only_lost_pass_form' ] ) ){
    $linkText   = isset( $config['link_text'] ) ? $config['link_text'] : __( 'Lost your password?', $userMeta->name );
...
$introText  = isset( $config['intro_text'] ) ? $config['intro_text'] : __('Please enter your username or email address. You will receive a link to create a new password via email.', $userMeta->name );
$html .= "<p>" . $introText . "</p>";
...
$html .= $userMeta->createInput( 'wp-submit', 'submit', array(
    'value'     => !empty( $config['button_value'] ) ? $config['button_value'] : __( 'Get New Password', $userMeta->name ),
