...
if ( !class_exists( 'GW_GoPricing' ) ) {
	class GW_GoPricing {
		...
		public function __construct() {
			...
			register_activation_hook( __FILE__, array( $this, 'go_pricing_activation' ) );
			register_uninstall_hook( __FILE__, array( $this, 'go_pricing_uninstall' ) );					
		...
		public function go_pricing_activation() {			
			$table_settings = get_option( GW_GO_PREFIX . 'table_settings' );
			$this->generate_styles ( $table_settings );
		}

		/* uninstall -> delete db data */
		public function go_pricing_uninstall() {			
		...
		public function go_pricing_load_css() {			
			...
			wp_register_style( GW_GO_PREFIX . 'skin_blue', plugin_dir_url( __FILE__ ) . 'assets/css/go_pricing_skin_blue.css', false, GW_GO_VER );
			wp_register_style( GW_GO_PREFIX . 'skin_green', plugin_dir_url( __FILE__ ) . 'assets/css/go_pricing_skin_green.css', false, GW_GO_VER );
			wp_register_style( GW_GO_PREFIX . 'skin_earth', plugin_dir_url( __FILE__ ) . 'assets/css/go_pricing_skin_earth.css', false, GW_GO_VER );
			wp_register_style( GW_GO_PREFIX . 'skin_red', plugin_dir_url( __FILE__ ) . 'assets/css/go_pricing_skin_red.css', false, GW_GO_VER );
			wp_register_style( GW_GO_PREFIX . 'skin_purple', plugin_dir_url( __FILE__ ) . 'assets/css/go_pricing_skin_purple.css', false, GW_GO_VER );
			wp_register_style( GW_GO_PREFIX . 'skin_yellow', plugin_dir_url( __FILE__ ) . 'assets/css/go_pricing_skin_yellow.css', false, GW_GO_VER );								
		}				

		public function generate_styles ( $data ) {
			...
			file_put_contents(plugin_dir_path( __FILE__ ) . 'assets/css/go_pricing_styles.css', $css_data);
		}
				...
				if ( !empty( $pricing_tables ) ) {
					...
					foreach ( $pricing_tables as $pricing_table ) {
						if 	( isset ( $pricing_table['table-id'] ) && $pricing_table['table-id'] == $id ) {
							...
							$generated_table='<div id="go-pricing-table-' . esc_attr( $pricing_table['table-id'] ) . '"' . ( $margin_bottom != '' ? ' style="margin-bottom:' . $margin_bottom . '"' : '' ) .'><div class="' . ( implode( $generated_table_classes, ' ' ) ) . '">';
							for ( $x = 0; $x < $colnum ; $x++ ) {
								list( $col_style ) = explode( '_', $pricing_table['col-style'][$x] );
								if ( preg_match( "/blue/", $col_style ) == 1 ) { wp_enqueue_style( GW_GO_PREFIX . 'skin_blue' ); 
								} elseif ( preg_match( "/green/", $col_style ) == 1 ) { wp_enqueue_style( GW_GO_PREFIX . 'skin_green' ); 
								} elseif ( preg_match( "/earth/", $col_style ) == 1 ) { wp_enqueue_style( GW_GO_PREFIX . 'skin_earth' );
								} elseif ( preg_match( "/red/", $col_style ) == 1 ) { wp_enqueue_style( GW_GO_PREFIX . 'skin_red' );
								} elseif ( preg_match( "/yellow/", $col_style ) == 1 ) { wp_enqueue_style( GW_GO_PREFIX . 'skin_yellow' );		
								} elseif ( preg_match( "/purple/", $col_style ) == 1 ) { wp_enqueue_style( GW_GO_PREFIX . 'skin_purple' ); }
								...
								for ( $y = $x*$rownum; $y < ( $x+1 )*$rownum ; $y++ ) {
									...
									$generated_table .= '<li' . ( !empty( $classes ) ? ' class="' . implode( ' ', $classes ) . '"' : '' ) . ( $pricing_table['col-align'][$y]!='' ? ' style="text-align:' . ( $pricing_table['col-align'][$y] ) . ';"' : '' ) .'>' . do_shortcode( $pricing_table['col-detail'][$y] ) . '<span class="gw-go-tooltip"' . ( !empty( $styles ) ? ' style="'. $style . '" ' : '' ) . '>' . ( isset( $pricing_table['col-detail-tip'][$y] ) && !empty( $pricing_table['col-detail-tip'][$y] ) ? do_shortcode( $pricing_table['col-detail-tip'][$y] ) : '' ) . '</span></li>';
									$row_count++;
								}
								$generated_table .=  '</ul>';				
								...
								if ( isset( $pricing_table['col-button-text'][$x] ) && ($pricing_table['col-button-text'][$x] != '' || $pricing_table['col-button-type'][$x] == 'custom' ) ) {
									if ( isset( $pricing_table['col-button-type'][$x] ) && $pricing_table['col-button-type'][$x] == 'button' ) {
										...
										$generated_table.='<span class="gw-go-btn gw-go-btn-' . ( $pricing_table['col-button-size'][$x] ) . '">' . do_shortcode( $pricing_table['col-button-text'][$x] ) . do_shortcode( $pricing_table['col-button-link'][$x] ) . '</a>';
                        ...
                        <li class="go-pricing-column">
                            ...
                            <div class="inside">
                                ...
                                <table class="form-table go-pricing-sortable-row">
                                    <tr>
	                                    <th class="w100">
                                        <input type="hidden" name="col-align[]" class="go-pricing-col-align" value="<?php echo isset( $pricing_tables[$uniqid]['col-align'][$x] ) ? $pricing_tables[$uniqid]['col-align'][$x] : ''; ?>" />
                                        	<a href="#" class="go-pricing-align-icon-left<?php echo isset( $pricing_tables[$uniqid]['col-align'][$x] ) && $pricing_tables[$uniqid]['col-align'][$x]=='left' ? ' go-pricing-current' : ''; ?>" data-id="left"></a>
                                        	<a href="#" class="go-pricing-align-icon-center<?php echo isset( $pricing_tables[$uniqid]['col-align'][$x] ) && $pricing_tables[$uniqid]['col-align'][$x]=='' ? ' go-pricing-current' : ''; ?>" data-id=""></a>
                                        	<a href="#" class="go-pricing-align-icon-right<?php echo isset( $pricing_tables[$uniqid]['col-align'][$x] ) && $pricing_tables[$uniqid]['col-align'][$x]=='right' ? ' go-pricing-current' : ''; ?>" data-id="right"></a>                                                                                       
                                        </th>
...
<?php _e( 'After clicking to the "Save" button your changes will appear in css file.', GW_GO_TEXTDOMAN ); ?>
		...
		public function go_pricing_settings_ajax_submit() {
			...
			if ( !isset( $response['result'] ) || $response['result'] != 'error' ) {
				...
				$this->generate_styles( $new_table_settings );
			}	
